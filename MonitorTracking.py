...
# ===== KEY STATE TRACKING WITH PYNPUT (replaces `keyboard` package; no sudo needed) =====
# We track held keys via pynput events, then read their state each frame.
key_state = {
    'j': False,   # yaw left
    'l': False,   # yaw right
    'i': False,   # pitch up
    ...
    '[': False,   # zoom out
    ']': False,   # zoom in
    'r': False,   # reset orbit view
    'f7': False,  # toggle mouse control
    'q': False,   # quit (optional; cv2.waitKey('q') also supported below)
}

def _pynput_key_to_char(key):
    """Return a comparable string for a key, or None."""
    try:
        return key.char
...
def update_orbit_from_keys():
    """Orbit camera controls driven by pynput key_state (prints when changed)."""
    ...
    # Rotate
    if key_state.get('j', False):  # yaw left
        orbit_yaw -= yaw_step; changed = True
    if key_state.get('l', False):  # yaw right
        orbit_yaw += yaw_step; changed = True
    if key_state.get('i', False):  # pitch up
        orbit_pitch += pitch_step; changed = True
    if key_state.get('k', False):  # pitch down
        orbit_pitch -= pitch_step; changed = True

    # Zoom
    if key_state.get('[', False):  # zoom out
        orbit_radius += zoom_step; changed = True
    if key_state.get(']', False):  # zoom in
...
while cap.isOpened():
    ...
    # F7 toggles mouse control
    if key_state.get('f7', False):
        mouse_control_enabled = not mouse_control_enabled
        print(f"[Mouse Control] {'Enabled' if mouse_control_enabled else 'Disabled'}")
        key_state['f7'] = False  # debounce (one toggle per press)

    # Quit if listener set q/ESC
    if key_state.get('q', False):
        break
